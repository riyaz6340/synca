...
export default function PersonsPage() {
  ...
  async function handleDeactivate(id: string) {
    ...
    try {
      await apiClient.patch(`/persons/${id}/deactivate`)
      void fetchPersons()
    } catch {
  ...
  async function handleCreateParent(id: string) {
    try {
      const res = await apiClient.post(`/persons/${id}/create-parent-account`, {})
  ...
  async function handleResetParentPwd(id: string) {
    try {
      const res = await apiClient.post(`/persons/${id}/reset-parent-password`, {})
      const creds = res.data.credentials
          ...
          <div style={modalStyle}>
            ...
            <div style={gridTwo}>
              <div>
                ...
                <input style={inputStyle} type="tel" value={form.parent_mobile} onChange={(e) => setForm({ ...form, parent_mobile: e.target.value })} placeholder="+91-9876543210" required />
              </div>
              <div>
