...
export default function PlatformDashboard() {
  ...
  const fetchOrganizations = useCallback(async () => {
    try {
      ...
      const res = await apiClient.get('/api/super-admin/organizations', { params });
  ...
  async function handleSavePlan() {
    ...
    setSaving(true);
    try {
      await apiClient.put(`/api/super-admin/organizations/${editingOrg.id}/plan`, {
        plan: editForm.plan,
      ...
      window.alert('Failed to update organization plan');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleAddOrg() {
    ...
    setAddingOrg(true);
    try {
      await apiClient.post('/api/super-admin/organizations', addOrgForm);
      setShowAddOrg(false);
          ...
          <tbody>
            {data.organizations_by_size.map(org => (
              <tr key={org.id || org.org_id}>
                <td style={tdStyle}><strong>{org.name || org.org_name}</strong></td>
                <td style={tdStyle}>{org.person_count}</td>
                <td style={tdStyle}><span style={{ ...planBadge, background: planColor(org.plan) }}>{org.plan || 'free'}</span></td>
