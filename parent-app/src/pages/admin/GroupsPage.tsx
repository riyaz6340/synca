...
export default function GroupsPage() {
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        await apiClient.put(`/api/groups/${editingId}`, form);
      } else {
        await apiClient.post('/api/groups', form);
      }
      setShowModal(false);
      void fetchGroups();
    } catch {
  ...
  async function openManageMembers(group: Group) {
    ...
    try {
      const res = await apiClient.get(`/api/groups/${group.id}`);
  ...
  async function addMemberDirect(personId: string) {
    ...
    try {
      await apiClient.post(`/api/groups/${managingGroup.id}/members`, { person_ids: [personId] });
      void openManageMembers(managingGroup);
      void fetchGroups();
    } catch (err: unknown) {
      const data = (err as { response?: { data?: { error?: string; conflicts?: string[] } } }).response?.data;
      if (data?.conflicts) {
        window.alert(`⚠️ Cannot add:\n\n${data.conflicts.join('\n')}`);
      } else {
        window.alert(data?.error || 'Failed to add member');
      }
    }
  }
  ...
  async function removeMember(personId: string) {
    if (!managingGroup) return;
    try {
      await apiClient.delete(`/api/groups/${managingGroup.id}/members/${personId}`);
      void openManageMembers(managingGroup);
      void fetchGroups();
    } catch {
                  ...
                  <div style={dropdownStyle}>
                    {persons
                      ...
                      .map(p => (
                        <div
                          key={p.id}
                          onClick={() => { setMemberSearch(''); setShowMemberDropdown(false); void addMemberDirect(p.id); }}
                          style={dropdownItem}
                        >
