...
export default function ReportsPage() {
  ...
  const fetchFilters = useCallback(async () => {
    try {
      const [groupsRes, personsRes] = await Promise.all([
        ...
        apiClient.get('/api/persons', { params: { limit: 100 } }),
      ]);
  ...
  function handleExport(format: 'pdf' | 'csv') {
    ...
    const token = localStorage.getItem('parent_app_token');
    window.open(`${apiClient.defaults.baseURL}/api/reports/attendance/export?${params.toString()}&token=${token ?? ''}`, '_blank');
  }
...
const inputStyle: React.CSSProperties = { padding: '0.45rem 0.6rem', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '0.85rem', width: '100%', boxSizing: 'border-box' as const };
