...
export default function HolidaysPage() {
  ...
  const fetchHolidays = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get('/api/holidays', { params: { year: currentYear } });
  ...
  async function handleAdd() {
    if (!form.date || !form.name) { window.alert('Date and name are required'); return; }
    setSaving(true);
    try {
      await apiClient.post('/api/holidays', form);
